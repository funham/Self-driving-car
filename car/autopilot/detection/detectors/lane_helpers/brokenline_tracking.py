...
class BrokenLineTracker:
    ...
    def get_distance_travelled(self, layout: cv2.Mat, out_img: cv2.Mat) -> float:
        ...
        cnts, _ = cv2.findContours(
            broken_line_image, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        bboxs = map(Rect, cnts)
        ...
        if len(bboxs) < 2 or not all([seg.is_correct for seg in curr_segments]):
            # couldn't find two bounding boxes, so assume no change in distance.
            return 0.0
        ...
        if cfg.DEBUG:
            for bbox in bboxs:
                cv2.drawContours(
                    out_img, [np.array(bbox.boxpoints) + [0, h//2]], 0, (0, 255, 0), 2)
