...
def eval_poly(coeff_vector, x):
    ...
    # [..., y^2, y^1, y^0]
    arg_vector = np.array([x] * (deg + 1)).astype(np.int64) ** power_vector
...
class LaneLines:
    def get_deviation_only_right(self, layout: cv2.Mat, out_img: cv2.Mat, p1, p2) -> float:
        ...
        for line in lines:
            ...
            cv2.line(out_img, (int(x0), y0), (img_w // 2, y0),
                     (0, 255, 255), 1, lineType=cv2.LINE_AA)
            cv2.line(out_img, (img_w // 2, y0), (img_w // 2, y0 - 20),
    ...
    def get_local_line_points(self, lane_layout: cv2.Mat):
        ...
        line = np.concatenate(
            [np.expand_dims(line_x, axis=0), np.expand_dims(line_y, axis=0)]).T
