...
class PerspectiveTransformation:
    ...
    def __init__(self, in_size: tuple, out_size: tuple,
        ...
        self.tl = np.array((self.in_w // 2 * (1 + toffset - twidth * wscale),
                           self.in_h * (1 - margin - height)))  # top left
        self.bl = np.array((self.in_w // 2 * (1 + boffset - bwidth * wscale),
                           self.in_h * (1 - margin)))           # bottom left
        self.tr = np.array(
            (self.tl[0] + self.in_w * twidth * wscale, self.tl[1]))  # top right
        self.br = np.array(
            (self.bl[0] + self.in_w * bwidth * wscale, self.bl[1]))  # bottom right
    ...
    def __call__(self, frame: cv2.Mat) -> cv2.Mat:
        ...
        transformed = cv2.warpPerspective(
            frame, self.M, (self.out_w, self.out_h), flags=cv2.INTER_LINEAR)
        ...
        lines = cv2.line(lines, p_mid_bottom, p_mid_top,
