...
from detection.detectors.lane_helpers.perspective_transform import PerspectiveTransformation
...
class RoadDetector(IDetector):
    ...
    def forward(self, frame: cv2.Mat) -> dict:
        ...
        distance_travelled = self.broken_line_tracker.get_distance_travelled(
            layout, out_img)
        crossroad_distance, p1, p2 = self.stopline_detector.get_stopline_distance(
            layout, out_img)
        deviation = self.lane_lines.get_deviation_only_right(
            layout, out_img, p1, p2)
